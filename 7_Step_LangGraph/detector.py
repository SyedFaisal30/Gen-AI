...
def detect_query(state: State):
    user_mesage = state.get("user_message")
    system_prompt = """
        You are a Ai Assistant or Agent that categorizes user queries, based on their user query you have to return a boolean value like true or false whether the question is realted to coding / programming or not.
    """
    messages = [
    ...
    state["is_coding_question"] = res.choices[0].message.parsed.is_query_related_to_coding
    return state
...
def answer_coding_question(state: State):
    ...
    system_prompt = """
        You are an helpfull AI Agent which solves user's question which is based on coding or ptogramming.
    """
...
def answer_simple_question(state: State):
    user_message = state.get("user_message")
    system_prompt = """
        You are an helpful AI agent which solves user's questions and answers them in a funny slangish genz way 
    """
